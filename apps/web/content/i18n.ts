...
export const APP_DESC: Record<SiteLocale, Record<string, string>> = {
  en: {
    ...
    "linktap": "Tap TCP and serial links — passive capture, proxy or bridge — with automatic Modbus and NMEA decoding.",
  },
  ko: {
    ...
    "linktap": "TCP·시리얼 링크 중간을 여는 탭 — 패시브 캡처·프록시·브리지. Modbus·NMEA 자동 해석.",
  },
  ja: {
    ...
    "linktap": "TCP・シリアルリンクの中間を開くタップ — パッシブキャプチャ・プロキシ・ブリッジ。Modbus・NMEAを自動解釈。",
  },
  de: {
    ...
    "linktap": "Tap für TCP- und serielle Verbindungen — passives Mitlesen, Proxy oder Bridge — mit automatischer Modbus- und NMEA-Dekodierung.",
  },
  zh: {
    "tdms-converter": "面向 NI TDMS 测量文件的桌面查看器与 CSV 转换工具。",
    frameterm: "面向帧级协议调试的串口终端。",
    "modbus-workbench": "带轮询、写入与内置模拟器的 Modbus RTU/TCP 主站。",
    "linktap": "打开 TCP 与串口链路中间的抓取器 — 被动抓包、代理或桥接，自动解析 Modbus 与 NMEA。",
  },
};
...
export const APP_DETAIL: Record<SiteLocale, AppDetailStrings> = {
  en: {
    ...
    apps: {
      ...
      linktap: {
        tagline: "Open the middle of a device link and read what passes — no separate install.",
        features: [
          "Passive capture (Windows): watch a link like Wireshark, capture driver bundled — admin only, nothing to install",
          "TCP proxy: point the client at LinkTap, it forwards to the real server — no driver",
  ...
  ko: {
    ...
    apps: {
      ...
      linktap: {
        tagline: "장치 링크의 중간을 열어 오가는 것을 읽습니다 — 별도 설치 없이.",
        features: [
          "패시브 캡처(Windows): Wireshark처럼 링크를 엿봄, 캡처 드라이버 내장 — 설치 없이 관리자 권한만",
          "TCP 프록시: 클라이언트를 LinkTap으로 향하게 하면 실제 서버로 전달 — 드라이버 불필요",
  ...
  ja: {
    ...
    apps: {
      ...
      linktap: {
        tagline: "デバイスリンクの中間を開き、流れるものを読みます — 別途インストール不要。",
        features: [
          "パッシブキャプチャ(Windows): Wiresharkのようにリンクを覗く、キャプチャドライバ内蔵 — インストール不要、管理者権限のみ",
          "TCPプロキシ: クライアントをLinkTapに向けると実サーバーへ転送 — ドライバ不要",
  ...
  de: {
    ...
    apps: {
      ...
      linktap: {
        tagline: "Öffnet die Mitte einer Geräteverbindung und liest mit — ohne separate Installation.",
        features: [
          "Passives Mitlesen (Windows): eine Verbindung wie Wireshark beobachten, Capture-Treiber integriert — keine Installation, nur Administratorrechte",
          "TCP-Proxy: Client auf LinkTap richten, Weiterleitung zum echten Server — kein Treiber",
  ...
  zh: {
    ...
    apps: {
      ...
      linktap: {
        tagline: "打开设备链路的中间，读取往来的数据 — 无需单独安装。",
        features: [
          "被动抓包(Windows)：像 Wireshark 一样观察链路，抓包驱动已内置 — 无需安装，仅需管理员权限",
          "TCP 代理：将客户端指向 LinkTap，由它转发到真实服务器 — 无需驱动",
