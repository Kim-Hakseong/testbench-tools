          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            {result.s.valid === null ? (
              <span className="rounded-full border border-warn/40 px-2.5 py-0.5 font-mono text-xs text-warn">
                no checksum — computed 0x{checksumHex(result.s.computed)}
              </span>
            ) : result.s.valid ? (
              <span className="rounded-full border border-ok/40 px-2.5 py-0.5 font-mono text-xs text-ok">
                checksum VALID (0x{checksumHex(result.s.computed)})
              </span>
            ) : (
              <span className="rounded-full border border-err/40 px-2.5 py-0.5 font-mono text-xs text-err">
                checksum FAIL — got 0x{checksumHex(result.s.received!)}, expected 0x{checksumHex(result.s.computed)}
              </span>
            )}
            {labels === undefined && (
              <span className="font-mono text-xs text-mute">unknown type — generic fields</span>
            )}
          </div>
              ...
              <tbody>
                {result.s.fields.map((f, i) => (
                  <tr key={i} className="border-b border-line-soft last:border-0">
                    ...
                    <td className="px-3 py-2 text-body">{labels?.[i] ?? `Field ${i + 1}`}</td>
