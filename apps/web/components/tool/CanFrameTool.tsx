...
export function CanFrameTool() {
  ...
  useEffect(() => {
    const h = setTimeout(() => setD({ idText, dataText }), 150);
    return () => clearTimeout(h);
  }, [idText, dataText]);

  const analysis = useMemo(() => {
    const idClean = d.idText.trim().replace(/^0x/i, "");
    if (!/^[0-9a-fA-F]{1,8}$/.test(idClean)) return { error: t("Identifier must be hex.") };
    const parsed = parseHex(rtr ? "" : d.dataText);
    if (!parsed.ok) {
      return {
        error: t("Data: invalid character at position {i}.").replace("{i}", String(parsed.error.index)),
      };
    }
      ...
      <div className="grid grid-cols-1 gap-5 md:grid-cols-2">
        <div className="space-y-3">
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label htmlFor="cf-id" className="font-mono text-xs text-mute">{t("Identifier (hex)")}</label>
          ...
          {!rtr && (
            <div>
              <label htmlFor="cf-data" className="font-mono text-xs text-mute">{t("Data bytes (hex, 0–8)")}</label>
        ...
        <div className="space-y-2.5">
          {"a" in analysis && analysis.a && analysis.a.errors.length === 0 && (
            <>
              <ResultCard label={`${t("Identifier")} (${t(analysis.a.extended ? "29-bit extended" : "11-bit standard")})`} value={analysis.a.idHex} size="lg" />
              <ResultCard label={t("Identifier (binary — lower wins arbitration)")} value={analysis.a.idBinary} />
              <ResultCard label={t("Identifier (decimal)")} value={String(analysis.a.id)} />
              <ResultCard label="DLC" value={String(analysis.a.dlc)} />
              {!analysis.a.rtr && analysis.a.data.length > 0 && (
                <ResultCard label={t("Data")} value={bytesToHex(analysis.a.data)} />
              )}
              {analysis.a.rtr && (
                <p className="rounded-btn border border-warn/40 px-3 py-2 font-mono text-xs text-warn">
                  {t("Remote frame — requests transmission of this identifier, carries no data.")}
                </p>
              )}
            </>
          )}
