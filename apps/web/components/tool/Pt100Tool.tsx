...
export function Pt100Tool() {
  ...
  useEffect(() => {
    const t = setTimeout(() => setD({ tempText, resText }), 150);
  ...
  const result = useMemo(() => {
    if (direction === "toR") {
      if (temp === null) return { error: "Enter a temperature." };
      const r = rtdResistance(temp, r0);
      if (r === null)
        return { error: "Supported range is 0…850 °C (the T < 0 °C branch of IEC 60751 is not implemented here)." };
      return { r };
    }
    if (res === null) return { error: "Enter a resistance." };
    const t = rtdTemperature(res, r0);
    if (t === null)
      return { error: `Supported range is R ≥ R0 (${r0} Ω), i.e. T ≥ 0 °C.` };
    return { t };
  }, [direction, temp, res, r0]);

  return (
    <div className="rounded-card border border-line-strong bg-surface p-4 sm:p-5">
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
        <div className="flex rounded-btn border border-line-strong p-0.5" role="tablist" aria-label="Sensor type">
          {(
            [
              [100, "PT100"],
              [1000, "PT1000"],
      ...
      <div className="mt-4 grid grid-cols-1 gap-5 md:grid-cols-2">
        <div>
          {direction === "toR" ? (
            <>
              <label htmlFor="pt-t" className="text-xs font-medium uppercase tracking-wide text-mute">Temperature (°C, 0…850)</label>
            ...
            <>
              <label htmlFor="pt-r" className="text-xs font-medium uppercase tracking-wide text-mute">Resistance (Ω, ≥ {r0})</label>
        ...
        <div className="space-y-2.5">
          {"r" in result && result.r !== undefined ? (
            <ResultCard label="Resistance" value={`${result.r.toFixed(4)} Ω`} size="lg" />
          ) : "t" in result && result.t !== undefined ? (
            <ResultCard label="Temperature" value={`${result.t.toFixed(4)} °C`} size="lg" />
          ) : (
