    ...
    <div className="rounded-card border border-line-strong bg-surface p-4 sm:p-5">
      ...
      {mode === "decode" ? (
        <div className="mt-4 grid grid-cols-1 gap-5 md:grid-cols-2">
          <div>
            <label htmlFor="cw-word" className="text-xs font-medium uppercase tracking-wide text-mute">{t("Command word (16-bit hex)")}</label>
            ...
            {decoded === null && <p className="mt-1.5 font-mono text-xs text-err">{t("Enter up to 4 hex digits.")}</p>}
            {decoded && (
              <p className="mt-3 font-mono text-xs text-mute">
                {t("bits 15–11 RT · 10 T/R · 9–5 subaddress · 4–0 word count / mode code")}
              </p>
            )}
          </div>
          <div className="space-y-2.5">
            {decoded && (
              <>
                <ResultCard label={decoded.broadcast ? t("RT address (broadcast)") : t("RT address")} value={String(decoded.rtAddress)} size="lg" />
                <ResultCard label="T/R" value={decoded.transmit ? t("1 — Transmit (RT → BC)") : t("0 — Receive (BC → RT)")} />
                <ResultCard label={t("Subaddress")} value={`${decoded.subaddress}${decoded.isModeCommand ? ` → ${t("mode command")}` : ""}`} />
                {decoded.isModeCommand ? (
                  <ResultCard label={`${t("Mode code")} ${decoded.modeCode}`} value={decoded.mode ? decoded.mode.name : t("reserved / undefined")} />
                ) : (
                  <ResultCard label={t("Word count")} value={`${decoded.wordCount} ${decoded.wordCount === 1 ? t("data word") : t("data words")}`} />
                )}
                <ResultCard label={t("Odd parity bit")} value={String(decoded.parity)} />
              </>
            )}
          </div>
        </div>
        ...
        <div className="mt-4 grid grid-cols-1 gap-5 md:grid-cols-2">
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label htmlFor="cw-rt" className="font-mono text-xs text-mute">{t("RT address (0–31)")}</label>
            ...
            <div>
              <label htmlFor="cw-sa" className="font-mono text-xs text-mute">{t("Subaddress (0–31)")}</label>
            ...
            <div className="col-span-2">
              <label className="font-mono text-xs text-mute">{t("Direction (T/R)")}</label>
            ...
            <div className="col-span-2">
              <label htmlFor="cw-wcm" className="font-mono text-xs text-mute">
                {isMode ? t("Mode code (0–31)") : t("Word count (1–32)")}
              </label>
              ...
              {isMode && <p className="mt-1 font-mono text-[11px] text-mute">{t("subaddress 0/31 → this field is a mode code")}</p>}
            </div>
          </div>
          <div className="space-y-2.5">
            <ResultCard label={t("Command word")} value={hex16(built)} size="lg" />
            <ResultCard label={t("Binary")} value={built.toString(2).padStart(16, "0")} />
            <ResultCard label={t("With odd parity (17-bit)")} value={`${built.toString(2).padStart(16, "0")} ${builtDecoded.parity}`} />
            {builtDecoded.isModeCommand && builtDecoded.mode && (
              <ResultCard label={`${t("Mode code")} ${builtDecoded.modeCode}`} value={builtDecoded.mode.name} />
            )}
          </div>
        </div>
      )}
