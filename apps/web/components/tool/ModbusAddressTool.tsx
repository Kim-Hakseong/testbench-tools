...
export function ModbusAddressTool() {
  ...
  const views = useMemo(() => {
    const t = d.trim();
    if (kind === "data-model") return parseDataModelAddress(t);
    if (!/^\d{1,5}$/.test(t)) return null;
    return modbusAddressViews(entity, Number(t));
  }, [kind, d, entity]);
    ...
    <div className="rounded-card border border-line-strong bg-surface p-4 sm:p-5">
      <div className="grid grid-cols-1 gap-5 md:grid-cols-2">
        <div className="space-y-3">
          ...
          {kind === "protocol" && (
            <div>
              <label htmlFor="ma-entity" className="text-xs font-medium uppercase tracking-wide text-mute">Entity type</label>
          ...
          <div>
            <label htmlFor="ma-addr" className="text-xs font-medium uppercase tracking-wide text-mute">
              {kind === "data-model" ? "Data-model address (e.g. 40011, 30001, 100005)" : "Protocol address (0-based, 0…65535)"}
            </label>
            ...
            {views === null && d.trim() !== "" && (
              <p className="mt-1.5 font-mono text-xs text-err" role="alert">
                {kind === "data-model"
                  ? "Use 5 or 6 digits starting with 0, 1, 3 or 4."
                  : "Enter an integer 0…65535."}
              </p>
            )}
          </div>
        </div>
        ...
        <div className="space-y-2.5">
          {views && (
            <>
              <ResultCard label="Entity" value={ENTITY_LABEL[views.entity]} />
              <ResultCard label="Protocol address (on the wire, 0-based)" value={String(views.protocol)} size="lg" />
              <ResultCard label="1-based address" value={String(views.oneBased)} />
              <ResultCard label="5-digit notation" value={views.fiveDigit ?? "— (offset > 9998)"} />
              <ResultCard label="6-digit notation" value={views.sixDigit} />
            </>
          )}
        </div>
      </div>
